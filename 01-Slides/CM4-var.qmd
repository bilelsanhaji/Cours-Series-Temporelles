...
```{r}
PolMon <- VAR(cbind(UNRATE.d, DFF.d, GDPDEF.dd), type = "none", p = 1)
# PolMon <- VAR(cbind(UNRATE.d, DFF.d, GDPDEF.dd), type = "none", p = 4)
...
Pour obtenir les 3 graphiques aller sur le .rmd
...
tseries::adf.test(donnees$log_c)
tseries::adf.test(donnees$log_a)
tseries::adf.test(donnees$log_y)
```
...
vecm_residuals <- residuals(vecm_var)
```

---

# Tests de diagnostic -- Corrélation sérielle
```{r}
# 1. Corrélation sérielle test
serial_test_result <- serial.test(vecm_var)
print(serial_test_result)
```

---

# Tests de diagnostic -- ARCH
```{r}
# 2. ARCH test (FinTS::ArchTest)
arch_test_result <- apply(vecm_residuals, 2, function(x) ArchTest(x, lags = 1))
print(arch_test_result)
```

---

# Tests de diagnostic -- Normalité
```{r}
# 3. Normalité test
normality_test_result <- normality.test(vecm_var)
...
plot(donnees$date, donnees$log_c, type = "l", col = "blue", lwd = 2,
     xlab = "Date", ylab = "Valeur", main = "Évolution des séries économiques", ylim = c(5,max(donnees$log_a)))
...
```{r}
#| echo: false
par(mfrow = c(ncol(vecm_residuals), 1))
for (i in 1:ncol(vecm_residuals)) {
  plot(vecm_residuals[, i], type = "l", main = paste("Résidus for Variable", i), ylab = "")
  abline(h = 0, col = "red", lty = 2)  # ligne à zero
}
```
...
par(mfrow = c(ncol(vecm_residuals), 2))  # Arrange plots in a grid
for (i in 1:ncol(vecm_residuals)) {
  acf(vecm_residuals[, i], main = paste("ACF de la Variable", i))
  pacf(vecm_residuals[, i], main = paste("PACF de la Variable", i))
}
```

---

# Graphiques - Histogramme et densité

```{r}
#| echo: false
# Histogram and density plots for residuals
par(mfrow = c(ncol(vecm_residuals), 2))  # Arrange plots in a grid
for (i in 1:ncol(vecm_residuals)) {
  hist(vecm_residuals[, i], breaks = 30, main = paste("Histogram of Residuals for Variable", i), xlab = "Residuals")
  plot(density(vecm_residuals[, i]), main = paste("Density Plot of Residuals for Variable", i), xlab = "Residuals")
}
```
...
par(mfrow = c(1, ncol(vecm_residuals)))  # Arrange plots in a row
for (i in 1:ncol(vecm_residuals)) {
  qqnorm(vecm_residuals[, i], main = paste("QQ Plot for Variable", i))
  qqline(vecm_residuals[, i], col = "red")  # Add a reference line
}
```
...
```{r}
#| echo: false
# Extract p-values from ARCH test results
arch_pvalues <- sapply(arch_test_result, function(x) x$p.value)

# Bar plot of ARCH test p-values
barplot(arch_pvalues, main = "ARCH Test P-Values", ylab = "P-Value", names.arg = paste("Variable", 1:ncol(vecm_residuals)))
abline(h = 0.05, col = "red", lty = 2)  # Add a significance level line
```
...
```{r}
#| echo: false
# Plot serial correlation test results
plot(serial_test_result)
```
...
irf_result <- irf(vecm_var, n.ahead = 10)
plot(irf_result)
```
