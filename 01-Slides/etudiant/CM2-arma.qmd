---
title: "Chapitre 2 — Processus ARMA stationnaires"
...
format:
  revealjs:
    footer: "Économétrie des séries temporelles — Chapitre 2"
---
...
```{r}
#| label: aic-bic
# DGP connu : AR(1) de coefficient 0.7. On compare trois spécifications.
set.seed(2627)
z <- arima.sim(list(ar = 0.7), n = 300)

m1 <- arima(z, order = c(1, 0, 0))   # le vrai modèle
m2 <- arima(z, order = c(2, 0, 0))   # sur-paramétré
m3 <- arima(z, order = c(4, 0, 1))   # nettement sur-paramétré

data.frame(
  modele = c("AR(1)", "AR(2)", "ARMA(4,1)"),
  p      = c(2, 3, 6),
...
Le BIC pénalise plus lourdement que l'AIC : l'écart entre les deux colonnes
s'accroît avec le nombre de paramètres. C'est pourquoi le BIC retrouve plus
souvent le vrai modèle, et l'AIC plus souvent un modèle trop riche.
